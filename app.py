...
class MIDImsg():
    ...
    def changeIn(self, relHpos):
        program = min(int(relHpos * 25) * 5, 127) 
        mf.midiout.send_message([0xC0 | self.chan, program])
        # self.NoteOn()
...
#                 if hand_sign_id == 0:
                ...
                #----|Right Hand|----
                    ...
                    if hand_sign_id == 1:
                        ...
                        ValV = [0, 0]
                        RightHand.NoteOff()
                        RightHand.changeIn(relHPos)
                    #--Pointer--
                    if hand_sign_id == 2:
                        key = (invrelVPos * 11) + 1
                        key = int(min(max(key - 1, 0), 11))
                        octave = int(relHPos * 9)
                        Note = key + (octave * 12) + 20
